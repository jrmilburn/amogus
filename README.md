...
- Crew: choose an unfinished task to highlight its current station on your private map. Mint diamonds mark your visible unfinished stations; white marks the tracked task, and amber means **Use** is ready within 80px. Tap **Use** on mobile or press **E** on a keyboard and follow the instrument instructions. **Close** (or Esc) cancels the attempt; long tasks retain earlier stages. Finished tasks leave the list, the remaining count decreases and the green crew bar updates for everyone. See the [desktop/mobile gameplay improvements and device checks](docs/design/gameplay-usability.md).
