...
- Gameplay tuning requested after the entry redesign: maximum player speed raised from 240 to 800px/s and default from 200 to 400px/s (latest user request); kill radius doubled from 60 to 120px and vent entry radius from 80 to 160px. Client targeting and server validation share the same constants; Impeccable-guided HUD/error hints now interpolate those constants. Wall, role and cooldown restrictions remain intact. All 95 tests, lint, typecheck and production build pass, including max-speed wall collision and expanded action-boundary regressions; the default-speed outer circuit now takes about 77 seconds. Changes are local, not committed or pushed; real-device balance acceptance remains pending.
